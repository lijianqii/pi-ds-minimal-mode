/**
 * Two-phase DeepSeek alignment extension for Pi.
 *
 ...
 * catalog.
 *
 * This is a minimal diff from hank9999/pi-ds-anchored: the only change is
 * extending the target model fragments to include deepseek-v4-flash.
 */
...
export default function dsAnchored(pi) {
  let ready = false;
  let promoted = false;
  ...
  pi.registerFlag("ds-anchored-promote-on", {
  ...
  const activateForTargetModel = (ctx) => {
    resetRuntimeState();
    promoteOn = parsePromoteOn(pi.getFlag("ds-anchored-promote-on"));
    try {
